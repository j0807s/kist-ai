...
class Trainer:
    def __init__(self, train_data, val_data, test_data, total_cls):
    ...
    def train(self, batch_size, epoches, lr, max_size):
        ...
        for inc_i in range(10):
            ...
            train_local = self.train_data[inc_i]
            val_local = self.val_data[inc_i]
            test_local = self.test_data[inc_i]
            print(len(train_local), len(val_local), len(test_local))
            train_x, train_y = zip(*train_local)
            val_x, val_y = zip(*val_local)
            test_x, test_y = zip(*test_local)
            ...
            exemplar.update(total_cls//10, (train_x, train_y), (val_x, val_y))
